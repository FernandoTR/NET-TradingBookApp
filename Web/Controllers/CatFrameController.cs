...
public class CatFrameController : Controller
{
    ...
    private static int permissionNumber = (int)Permissions.CatFrame;

    public CatFrameController(IIdentityService identityService,
    ...
    {
        var data = new List<CatFrameViewModel>();
        ...
        try
        {
            ...
            var query = (await _catFrameService
                        .GetAllAsync())
                        .Select(x => new CatFrameViewModel
                        {
                            Id = x.Id,
                            Code = x.Code,
        ...
        catch (Exception ex)
        {
            ...
            _logService.ErrorLog($"Controller: CatFrame, Action: JsonDataTable", ex);
        }
    ...
    public IActionResult Index(NotificationType? notification, string message)
    {
        ...
        {
            _logService.ErrorLog($"Controller: CatFrame, Action: {nameof(Index)}", ex);
            return RedirectToAction("index", "Home");
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Save(CatFrameViewModel model)
    {
        ...
        try
        {
            var figure = new CatFrame()
            {
                Code = model.Code.Trim(),
                Description = model.Description.Trim(),
            };

            if (await _catFrameService.AddAsync(figure))
            {
                notification = NotificationType.Success;
                message = string.Format(_messageService.GetResourceMessage("FrameSuccessfullySaved"), model.Description);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            message = _messageService.GetResourceError("GenericError");
            notification = NotificationType.Error;
            _logService.ErrorLog($"Controller: CatFrame, Action: {nameof(Save)}", ex);
        }
    ...
    public async Task<IActionResult> Edit(int id)
    {
        ...
        CatFrameViewModel model = null;
        try
        {
            var frame = await _catFrameService.GetByIdAsync(id);
            model = new CatFrameViewModel()
            {
                Id = frame.Id,
                Code = frame.Code,
        ...
        catch (Exception ex)
        {
            ...
            _logService.ErrorLog($"Controller: CatFrame, Action: {nameof(Edit)}", ex);
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Update(CatFrameViewModel model)
    {
        // Recuperar el usuario logueado
        var currentUser = _identityService.GetCurrentUserAsync();
        ...
        try
        {
            ...
            if (await _catFrameService.UpdateAsync(frame))
            {
                notification = NotificationType.Success;
                message = string.Format(_messageService.GetResourceMessage("FrameSuccessfullyUpdated"), model.Description);
            }
            else
            {
                message = _messageService.GetResourceError("GenericError");
        ...
        catch (Exception ex)
        {
            message = _messageService.GetResourceError("GenericError");
            notification = NotificationType.Error;
            _logService.ErrorLog($"Controller: CatFrame, Action: {nameof(Update)}", ex);
        }
    ...
    public async Task<IActionResult> Delete([FromBody] int id)
    {
        ...
        try
        {
            bool isDeleted = await _catFrameService.DeleteAsync(id);
