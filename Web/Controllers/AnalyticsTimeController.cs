...
[Authorize]
public class AnalyticsTimeController : Controller
{
    ...
    private static int permissionNumber = (int)Permissions.AnalyticsDay;

    public AnalyticsTimeController(IIdentityService identityService,
    ...
    {
        var data = new List<GetTBAnalyticsTimeDto>();
        ...
        try
        {
            ...
            var query = await _ordersService.GetTBAnalyticsTimeAsync(new ParametersTBAnalyticsDto
            {
        ...
        catch (Exception ex)
        {
            ...
            _logService.ErrorLog($"Controller: AnalyticsTime, Action: JsonDataTable", ex);
        }
    ...
    public async Task<IActionResult> Index(NotificationType? notification, string message)
    {
        ...
        {
            _logService.ErrorLog($"Controller: AnalyticsTime, Action: {nameof(Index)}", ex);
            return RedirectToAction("index", "Home");
        }
