...
public class CatSceneryRepository : ICatSceneryRepository
{
    private readonly ILogService _logService;
    private readonly ApplicationDbContext _context;

    public CatSceneryRepository(ApplicationDbContext context, ILogService logService)
    {
        _context = context;
    ...
    /// <summary>
    ///  Obtiene una lista de escenarios para cargar un DataTable en la aplicación.
    /// </summary>
    /// <param name="parameters">Modelo con los parámetros necesarios para ejecutar el procedimiento almacenado.</param>
    /// <returns>Una lista de objetos <see cref="GetTBAnalyticsSceneryDto"/> correspondientes a los escenarios.</returns>
    public async Task<List<GetTBAnalyticsSceneryDto>> GetTBAnalyticsSceneryAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            var result = await _context.Set<GetTBAnalyticsSceneryDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsScenery @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @DirectionId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
        ...
        {
            _logService.ErrorLog(nameof(GetTBAnalyticsSceneryAsync), ex);
            return new List<GetTBAnalyticsSceneryDto>();
        }
    ...
    /// </summary>
    /// <param name="parameters">Modelo con los parámetros necesarios para ejecutar el procedimiento almacenado.</param>
    /// <returns>Una lista de objetos <see cref="GetTBAnalyticsSceneryDto"/> correspondientes a los escenarios con calificación automática.</returns>
    public async Task<List<GetTBAnalyticsSceneryDto>> GetTBAnalyticsSceneryAutoScoreAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            // Ejecutar el procedimiento almacenado y obtener los resultados
            var result = await _context.Set<GetTBAnalyticsSceneryDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsSceneryAutoScore @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @DirectionId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
