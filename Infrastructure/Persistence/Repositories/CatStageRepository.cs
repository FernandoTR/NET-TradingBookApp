    ...
    public async Task<List<GetTBAnalyticsStageDto>> GetTBAnalyticsStageAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            var result = await _context.Set<GetTBAnalyticsStageDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsStage @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @DirectionId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
