...
public class CatDirectionRepository : ICatDirectionRepository
{
    private readonly ILogService _logService;
    private readonly ApplicationDbContext _context;

    public CatDirectionRepository(ApplicationDbContext context, ILogService logService)
    {
        _context = context;
    ...
    /// <summary>
    ///  Obtiene una lista de direciones para cargar un DataTable en la aplicación.
    /// </summary>
    /// <param name="parameters">Modelo con los parámetros necesarios para ejecutar el procedimiento almacenado.</param>
    /// <returns>Una lista de objetos <see cref="GetTBAnalyticsDirectionDto"/> correspondientes a las direcciones.</returns>
    public async Task<List<GetTBAnalyticsDirectionDto>> GetTBAnalyticsDirectionAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            var result = await _context.Set<GetTBAnalyticsDirectionDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsDirection @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
        ...
        {
            _logService.ErrorLog(nameof(GetTBAnalyticsDirectionAsync), ex);
            return new List<GetTBAnalyticsDirectionDto>();
        }
