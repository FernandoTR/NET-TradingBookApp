...
public class CatDayRepository : ICatDayRepository
{
    private readonly ILogService _logService;
    private readonly ApplicationDbContext _context;

    public CatDayRepository(ApplicationDbContext context, ILogService logService)
    {
        _context = context;
    ...
    /// <summary>
    ///  Obtiene una lista de días para cargar un DataTable en la aplicación.
    /// </summary>
    /// <param name="parameters">Modelo con los parámetros necesarios para ejecutar el procedimiento almacenado.</param>
    /// <returns>Una lista de objetos <see cref="GetTBAnalyticsDayDto"/> correspondientes a los días.</returns>
    public async Task<List<GetTBAnalyticsDayDto>> GetTBAnalyticsDayAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            var result = await _context.Set<GetTBAnalyticsDayDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsDay @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @DirectionId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
        ...
        {
            _logService.ErrorLog(nameof(GetTBAnalyticsDayAsync), ex);
            return new List<GetTBAnalyticsDayDto>();
        }
