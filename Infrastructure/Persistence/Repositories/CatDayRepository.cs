    ...
    public async Task<List<GetTBAnalyticsDayDto>> GetTBAnalyticsDayAsync(ParametersTBAnalyticsDto parameters)
    {
        try
        {
            ...
            var result = await _context.Set<GetTBAnalyticsDayDto>()
                .FromSqlRaw("EXEC usp_GetTBAnalyticsDay @CategoryId, @AccountTypeId, @InstrumentId, @FrameId, @SearchValue, @OrderByColumn, @SortColumnDir, @Skip, @Take, @Count OUTPUT", sqlParameters)
                .ToListAsync();
